...
<VirtualHost *:80>
    ...
    ServerAdmin support@EXAMPLE_COM
    ServerName  EXAMPLE_COM
    ServerAlias XXXX.EXAMPLE_COM

    # Redirect permanent / https://XXXX.EXAMPLE_COM
...
> To be skipped
```
sudo mkdir /usr/local/www/apache24/data/EXAMPLE_COM
sudo chown -R www:www /usr/local/www/apache24/data/EXAMPLE_COM
```
...
```
sudo certbot --apache -d EXAMPLE_COM -d XXXX.EXAMPLE_COM
```
...
```
RewriteEngine on
RewriteCond %{SERVER_NAME} =XXXX.EXAMPLE_COM [OR]
RewriteCond %{SERVER_NAME} =EXAMPLE_COM
...
SSLCertificateFile /usr/local/etc/letsencrypt/live/EXAMPLE_COM/fullchain.pem
SSLCertificateKeyFile /usr/local/etc/letsencrypt/live/EXAMPLE_COM/privkey.pem
</VirtualHost>
</IfModule>
```
...
ServerName XXXX.EXAMPLE_COM
## >>> EXAMPLE_COM when no sub-domains
...
<VirtualHost *:443>
    ...
    WSGIDaemonProcess ariel python-path=/FOO/BAR/BAZ/<PROJECT_SLUG>:/FOO/BAR/BAZ/<PROJECT_SLUG>/venv/lib/python3.9/site-packages/ user=<REMOTE_USERNAME> group=<REMOTE_USERNAME> processes=2 threads=25
    WSGIProcessGroup  ariel
    ...
    SSLCertificateFile    /usr/local/etc/letsencrypt/live/XXXX.EXAMPLE_COM/fullchain.pem
    SSLCertificateKeyFile /usr/local/etc/letsencrypt/live/XXXX.EXAMPLE_COM/privkey.pem
    Include               /usr/local/etc/letsencrypt/options-ssl-apache.conf
...
```
sudo mkdir -p /usr/local/docs/EXAMPLE_COM
```
